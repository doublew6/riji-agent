...
errors. ``provider_label`` only affects error text so a failure names the
configured provider without ever echoing the request body or credentials.
"""
...
class OpenAICompatibleProvider:
    ...
    def complete(
        ...
        try:
            ...
            raise LLMError(
                f"{self._label} request failed with status {exc.response.status_code}"
            ) from None
        except httpx.HTTPError:
            raise LLMError(f"{self._label} request failed") from None
        except ValueError:
            raise LLMError(f"{self._label} returned a malformed response") from None
    ...
    def _parse(data: Dict[str, Any], label: str) -> AssistantTurn:
        try:
            ...
            raise LLMError(f"{label} returned no choices") from None
