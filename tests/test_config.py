...
def test_settings_accepts_voxcpm_provider(tmp_path: Path) -> None:
    journal_root = tmp_path / "journal"
    journal_root.mkdir()
    ...
    settings = Settings(
        ...
        RIJI_TTS_CFG_VALUE=2.5,
        RIJI_TTS_INFERENCE_TIMESTEPS=12,
    )

    assert settings.tts_provider == "voxcpm"
    assert settings.tts_model == "openbmb/VoxCPM2"
    assert settings.tts_cfg_value == 2.5
    assert settings.tts_inference_timesteps == 12
