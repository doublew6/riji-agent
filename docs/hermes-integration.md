...
若需要更自然的本地开源音色，可选 `voxcpm`。它基于 VoxCPM2，支持给每个导师
配置自然语言音色描述，不需要真人参考音频：
...
语音回复仍然只在用户明确要求“用语音/发语音/朗读”等触发词时生成；普通回复保持文字。

### 可选：飞书日历

日历 provider 默认关闭。启用后，飞书私聊里的自然语言日程请求会先生成本地草稿
预览；只有用户回复「确认创建」后，riji-agent 才调用飞书日历 API 创建日程，
并把发起人加入参与人列表，让日程出现在用户的飞书日历里。当天日程会写入当天
daily note；未来日程不会提前创建未来 daily note，会等到当天再按当时模板记录。
...
飞书开放平台权限以 [feishu-permissions.yaml](feishu-permissions.yaml) 为唯一
维护清单。启用或关闭语音、日历等能力时，先更新该文件，再同步开发者后台权限。
...
1. 在 Hermes 一侧按官方文档接入飞书自建应用，并按 [feishu-permissions.yaml](feishu-permissions.yaml) 开通所需权限；默认仅启用**私聊**消息接收事件，确保事件携带稳定的 `event_id`。
