...
| 决策 | 结论 |
| --- | --- |
| 飞书入口 | 使用一个 `Riji` Bot；用户在同一 Bot 内通过命令或按钮切换导师。 |
| 推理与编排 | Hermes 使用 DeepSeek 执行多轮 agent 推理、工具调用、飞书会话路由与定时任务。 |
| 数据边界 | riji-agent 是唯一可读写日记、记忆、草稿和审计库的本地服务；Hermes 不直接拥有 vault 权限。 |
| 记忆 | 日记与已确认长期记忆共享；导师会话历史和未确认工作记忆隔离。 |
| 内容排除 | MVP 不设置 `private: true`；所有已索引日记均可被检索。系统坚持最小片段出云，不上传完整 vault。 |
...
```mermaid
flowchart LR
  U["用户：飞书私聊"] --> F["Riji Bot"]
  F <--> H["Hermes\n飞书接入、路由、DeepSeek Agent"]
  H <--> D["DeepSeek API\n推理与工具选择"]
  H <--> R["riji-agent\n受限工具与权限内核"]
  R <--> S["SQLite\n索引、记忆、草稿、审计"]
...
| 组件 | 负责 | 明确不负责 |
| --- | --- | --- |
| 飞书 / Riji Bot | 接收私聊、展示导师选择和草稿预览 | 存放日记、持有模型密钥 |
| Hermes | 内置飞书接入、路由、会话、DeepSeek 多轮调用、cron/skills | 直接读写 vault、保存权威长期事实 |
| riji-agent | 工具实现、权限、本地索引、草稿状态机、原子写入、审计 | 飞书协议、任意终端或任意文件读取 |
| SQLite | 索引元数据、会话映射、草稿、已确认记忆、审计事件 | 原始 vault 的替代副本 |
| DeepSeek | 规划检索、调用已注册工具、基于证据回答 | 访问完整 vault、任意路径、未注册工具 |

Hermes 与 riji-agent 在同一台主机上通信。riji-agent 默认只监听 localhost，且以配置明确的 vault 路径只读打开日记源。
...
没有单篇排除开关意味着：飞书消息会经过飞书服务；DeepSeek 会收到问题、系统提示和检索命中的少量日记片段。完整 vault、原始 Markdown 文件和本地 SQLite 不上传。
...
src/riji_agent/
  api/          # Hermes 可调用接口
  auth/         # 用户、会话、能力校验
  journal/      # 解析、模板、索引、原子追加
  retrieval/    # FTS、向量、时间线
  memory/       # 共享/私有记忆与候选审核
  personas/     # 人设、王阳明资料访问策略
  drafts/       # patch、状态机、确认 token
  audit/        # 审计事件
```

依赖只能由外向内：`api → auth / personas / drafts → journal / retrieval / memory / audit`。`journal` 不依赖 Hermes、飞书或 DeepSeek。
