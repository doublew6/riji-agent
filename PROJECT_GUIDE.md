...
构建一个通过 Hermes 内置飞书接入交互的日记 Agent：可配置的模型适配器负责多轮推理（默认 DeepSeek，可选 Codex），`riji-agent` 负责本地日记检索、记忆、草稿确认、模板追加与审计。导师与记忆模型可分别配置，接入边界见 `docs/codex-provider.md`。
...
- 遵循已确认架构和长期记忆 PRD：排除 `private: true` 内容；保留单片段 900 字符、来源版本累计 4000 字符及来源审计。用户于 2026-09-09 明确确认日常处理为每 UTC 日 100000 字符；本次固定来源版本批次的历史初始化不设每日总量上限，独立计量并在批次结束后自动终止初始处理豁免。用户于 2026-09-10 进一步确认：本固定批次已显式登记、绑定失败 ID / 版本的一次整理恢复也使用初始化账本；恢复达到终态后关闭对应例外，不重新开放原批次、扩充清单或自动再次重试。此例外不放宽 private/local/none、四种用途授权、单次/来源限制或后续增量预算，不得实现为永久无限开关。

## 飞书、导师与记忆

以下单 Bot 与私聊限制是既有 MVP 运行基线。用户于 2026-09-10 已确认下一阶段“固定导师独立私聊 + 受控私人圆桌群 + 渠道解耦”的设计方向，详见 `docs/product/mentor-dialogue-and-debate.md` 与 `docs/architecture/mentor-dialogue-and-debate.md`。对应实现 Issue 须显式建立新身份、群成员/用途授权和投递边界并通过验收，才能启用私人圆桌路径；普通群、未核验群与旧入口仍不得访问群日记能力，群内仍不能创建或提交日记草稿。不得仅修改 chat_type 或白名单来提前放行。
